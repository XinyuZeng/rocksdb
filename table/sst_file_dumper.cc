...
Status SstFileDumper::DumpTable(const std::string& out_filename) {
  ...
  s = env->NewWritableFile("../key.txt", &out_file_key, soptions_);
  if (!s.ok()) {
    // close the file before return error, ignore the close error if there's any
    out_file_key->Close().PermitUncheckedError();
    return s;
  }
  s = env->NewWritableFile("../offset.txt", &out_file_offset, soptions_);
  if (!s.ok()) {
    // close the file before return error, ignore the close error if there's any
    out_file_offset->Close().PermitUncheckedError();
    return s;
  }
  s = env->NewWritableFile("../size.txt", &out_file_size, soptions_);
  if (!s.ok()) {
    // close the file before return error, ignore the close error if there's any
